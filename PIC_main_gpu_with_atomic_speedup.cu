...
double* ParticleDeposition_GPU(particle* h_point, long int n);
...
__global__ void kernel(particle* d_point, double *d_rho,long int n)
{
    ...
    if(index<n)
    {
        ...
        double tx  = (d_point[index].xcoord + xn_max)/dXn;
        int itx = (d_point[index].xcoord + xn_max)/dXn;
        double wx2 = tx - itx;
        double wx1 = 1.0 - wx2;


        double ty  = (d_point[index].ycoord + yn_max)/dYn;
        int ity = (d_point[index].ycoord + yn_max)/dYn;
        double wy2 = ty - ity;
        double wy1 = 1.0 - wy2;
       ...
       /*  
        d_rho[(iy*(N))+(ix)]      += wx1 * wy1;
        d_rho[(iy*(N))+((ix+1))]    += wx2 * wy1;
        d_rho[((iy+1)*(N))+(ix)]    += wx1 * wy2;
        d_rho[((iy+1)*(N))+(ix+1)]  += wx2 * wy2;*/
        

        atomicAdd (&d_rho[(iy*(N))+(ix)], wx1 * wy1);
        atomicAdd (&d_rho[(iy*(N))+((ix+1))] , wx2 * wy1);
        atomicAdd (&d_rho[((iy+1)*(N))+(ix)], wx1 * wy2);
        atomicAdd (&d_rho[((iy+1)*(N))+(ix+1)], wx2 * wy2);
...
int main()
{
    ...
    rho_gpu=ParticleDeposition_GPU(h_point,  n);
    double *diff_rho;
...
double* ParticleDeposition_GPU(particle* h_point, long int n)
{
  ...
  for (int i=0; i<(N+1)*(N+1); i++)
  {
    ...
    cudaMemcpy(d_point,h_point, n*sizeof(particle), cudaMemcpyHostToDevice);
     ...
     kernel<<<grid_size,block_size>>>(d_point,d_rho,n);
